...
let LolliComponent = class LolliComponent {
    constructor() {
        this.data = d.default;
        this.wd_dict = { "w1": 'Sunday', "w2": 'Monday', "w3": 'Tuesday', "w4": 'Wednesday', "w5": 'Thursday', "w6": 'Friday', "w7": 'Saturday' };
        this.svgWidth = 900;
        this.svgHeight = 450;
        this.margin = { top: 50, right: 25, bottom: 60, left: 90 };
    ...
    redraw(srcEle) {
        if (srcEle.nodeName == "BUTTON") {
            ...
            var pre = cid.slice(0, 3), suf = cid.slice(4, 6);
            if (pre == "evl") {
                if (suf == "wd") {
                    this.setEvlMode(".evl-wd");
                    this.dataset = this.data.weekday;
                ...
                if (suf == "wd") {
                    var dataset = [];
                    this.dataset.forEach(x => dataset.push(Object.assign({}, x)));
    ...
    setTitle() {
        var evl = D3.select(".evl .active").text(), srt = D3.select(".order .active").text();
...
], LolliComponent.prototype, "redraw", null);
LolliComponent = __decorate([
    Component({
        selector: 'app-lolli',
...
export { LolliComponent };
//# sourceMappingURL=lolli.component.js.map